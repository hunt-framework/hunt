...
main = do
  ...
  mapM (\(plot,file) -> renderableToFile (FileOptions (640,400) SVG M.empty) (toRenderable plot) file) 
    [ (memory1, "memory1.svg")
    , (memory2, "memory2.svg")
    , (time1  , "time1.svg")
    , (time2  , "time2.svg")
...
lineDiagram :: String -> [[(Int, Double)]] -> PlotLines Int Double
lineDiagram title res = 
  def & plot_lines_title  .~ title
      & plot_lines_style  .~ def
      & plot_lines_values .~ res
